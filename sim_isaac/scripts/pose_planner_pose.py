...
class PosePlanner(object):
    def __init__(self, filepath):
        ...
        self.pos_list = np.vstack([self.pos_list, self.pos_list, self.pos_list, self.pos_list])
        self.pos_list[0, 2] = self.pos_list[0, 2] + 0.12
        self.pos_list[1, 2] = self.pos_list[1, 2] + 0.05
        self.pos_list[2, 2] = self.pos_list[2, 2] + 0.12
        self.pos_list[3, 2] = self.pos_list[3, 2] + 0.15
        self.ori_list = data['matrix']
        self.ori_list = np.vstack([[self.ori_list], [self.ori_list], [self.ori_list], [self.ori_list]])
        
        
        self.quat_list = data['quat']
        self.rpy_list = data['rpy']
        
        self.quat_list = np.vstack([self.quat_list, self.quat_list, self.quat_list, self.quat_list])
        self.rpy_list = np.vstack([self.rpy_list, self.rpy_list, self.rpy_list, self.rpy_list])
        
        # self.rpy_list[0, 2] = self.rpy_list[0, 2] + 0.12
        # self.rpy_list[1, 2] = self.rpy_list[1, 2] + 0.05
        self.rpy_list[2, 1] = self.rpy_list[2, 1] + np.deg2rad(-25)
        self.rpy_list[3, 1] = self.rpy_list[3, 1] + np.deg2rad(-50)
        ...
        self.flag_gripper_control = False
    ...
    def calc(self):
        ...
        try:
            ...
            moveit_commander.roscpp_initialize(sys.argv)
            group = "arm"
            if self.flag_gripper_control:
                group = "gripper"
                self.arm_move_group = moveit_commander.MoveGroupCommander(group)
                self.arm_move_group.set_planning_time(10.0)
                # self.arm_move_group.set_goal_position_tolerance(0.005)
                # self.arm_move_group.set_goal_orientation_tolerance(0.005)
                # self.arm_move_group.set_num_planning_attempts(10) 
                ...
                # # エンドポイントの姿勢でゴール状態を指定
                self.arm_move_group.set_joint_value_target(np.deg2rad([27.5]))    
                # # # モーションプランの計画と実行
                success = self.arm_move_group.go(wait=True)
                self.flag_gripper_control = False
                ...
                self.arm_move_group.set_planning_time(10.0)
                self.arm_move_group.set_goal_position_tolerance(0.001)
                self.arm_move_group.set_goal_orientation_tolerance(0.001)
                self.arm_move_group.set_num_planning_attempts(10) 
                self.arm_move_group.set_goal_position_tolerance(0.001)
                self.arm_move_group.set_goal_orientation_tolerance(0.001)
                self.arm_move_group.set_planner_id("RRTConnectkConfigDefault")
                ...
                (trans_x, trans_y, trans_z) = self.pos_list[i]
                # (ori_r, ori_p, ori_y) = rt.rotmat2rpy(self.ori_list[i])
                (ori_r, ori_p, ori_y) = self.rpy_list[i]
                pose_goal.position = Vector3(trans_x, trans_y, trans_z)
                q = tf.transformations.quaternion_from_euler(ori_r, ori_p, ori_y)
                # q = self.quat_list[i]
                pose_goal.orientation = Quaternion(x=q[0], y=q[1], z=q[2], w=q[3])
            ...
            if self.count == 1:
                self.flag_gripper_control = True
            
        except BaseException:
            pass

        if self.count >= len(self.pos_list) - 1:
            self.count += 0 
        else:
            self.count += 1
...
if __name__ == '__main__':
    ...
    # process1 = multiprocessing.Process(target=mock.pub_cameras_tf, args=("world", "cube_mesh", mock.pos_list, mock.quat_list))
